...
public class FileAddController implements Controller{
	...
	public String requestHandler(HttpServletRequest request, HttpServletResponse response)
		...
		// file 업로드 시 필요한 API - commons-fileupload, commons-io
		...
		try {
			List<FileItem> items=upload.parseRequest(request); // request 안에 여러개의 파일이 업로드된 경우..
			for(int i=0;i<items.size();i++) {
				...
				if(fileItem.isFormField()) { // 폼필드이면
					...
					if(fileItem.getSize()>0) {
						int idx=fileItem.getName().lastIndexOf("\\"); // \\(윈도우), /(리눅스)
						if(idx==-1) {
							idx=fileItem.getName().lastIndexOf("/");
						}
						fileName=fileItem.getName().substring(idx+1); // 마지막 / 다음이 파일명
