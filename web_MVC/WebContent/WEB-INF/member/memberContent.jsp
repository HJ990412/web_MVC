    	...
    	if($("#file").val()!=''){ //파일이 첨부가 된경우...
   		 ...
   		 $.ajax({
   			 ...
   			 success : function(data){  //업로드된 파일 이름 전달받기
   				 //alert(data);
   				 $('#filename').val(data);
   			     document.form1.action="<c:url value='memberUpdate.do'/>?mode=fupdate"; // text 데이터 저장
   		  ...
   		  document.form1.action="<c:url value='memberUpdate.do'/>?mode=update"; // text 데이터 저장
    ...
    <div class="panel-heading">
      <c:if test="${sessionScope.userId!=null && sessionScope.userId!='' && sessionScope.userId==vo.id}">
  		<label>
  		     ...
  		     ${sessionScope.userName}님이 로그인 하셨습니다..</label>
 	  </c:if>
